...
def normal_block(input, filters, stage, block, use_bias=True, train_bn = True):
    ...
    x = layers.Conv2D(filters=filter1, kernel_size=(1,1),name=conv_name_base + '2a', use_bias=use_bias)(input)
    x = BatchNorm(name=bn_name_base + '2a')(x, training=train_bn)
    x = layers.Activation('relu')(x)

    x = layers.Conv2D(filters=filter2, kernel_size=(3,3), padding='same', name=conv_name_base + '2b', use_bias=use_bias)(x)
    x = BatchNorm(name=bn_name_base + '2b')(x, training=train_bn)
    x = layers.Activation('relu')(x)

    x = layers.Conv2D(filters=filter3, kernel_size=(1,1), name=conv_name_base + '2c', use_bias=use_bias)(x)
...
def halved_block(input, filters, stage, block, strides=(2,2), use_bias=True, train_bn = True):
    ...
    shortcut = layers.Conv2D(filters=filter3, kernel_size=(1,1), strides=strides, name=conv_name_base + '1', use_bias=use_bias)(input)
...
def build_layers(input, train_bn=True):
    ...
    x = layers.Conv2D(filters=64, kernel_size=(7,7), strides=(2,2), name='conv1', use_bias=True)(x)
...
def build_FPN(C1,C2,C3,C4,C5,config):
    P5 = layers.Conv2D(config.PIRAMID_SIZE, kernel_size=(1,1), name='fpn_c5p5')(C5)

    P4 = layers.Add(name="fpn_p4add")([
        layers.UpSampling2D(size=(2,2), name="fpn_p5upsampled")(P5),
        layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(1,1), name='fpn_c4p4')(C4)
    ])

    P3 = layers.Add(name="fpn_p3add")([
        layers.UpSampling2D(size=(2,2), name="fpn_p4upsampled")(P4),
        layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(1,1), name='fpn_c3p3')(C3)
    ])

    P2 = layers.Add(name="fpn_p2add")([
        layers.UpSampling2D(size=(2,2), name="fpn_p3upsampled")(P3),
        layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(1,1), name='fpn_c2p2')(C2)
    ])

    P2 = layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(3,3), padding="SAME", name="fpn_p2")(P2)
    P3 = layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(3,3), padding="SAME", name="fpn_p3")(P3)
    P4 = layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(3,3), padding="SAME", name="fpn_p4")(P4)
    P5 = layers.Conv2D(filters=config.PIRAMID_SIZE, kernel_size=(3,3), padding="SAME", name="fpn_p5")(P5)
