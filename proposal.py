...
class ProposalLayer(layers.Layer):
    ...
    def call(self,input):
        ...
        ids = tf.math.top_k(input=class_probs, k=pre_nms_limit, sorted=True,
                            name="top_anchors").indices #find k largest probabilities

        #slice to each batch ( images per process)
        class_probs = utils.batch_slice([class_probs, ids], lambda x,y:tf.gather(params=x, indices=y),
                                        self.config.IMAGES_PER_GPU)
        bbox_offset = utils.batch_slice([bbox_offset, ids], lambda x,y:tf.gather(params=x, indices=y),
                                        self.config.IMAGES_PER_GPU)
